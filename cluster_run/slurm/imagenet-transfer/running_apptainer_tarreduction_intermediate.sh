...
mv ${LOCAL_JOB_DIR}/output/${seed}/ckpts/transfer-model-ep-31.pt ${LOCAL_JOB_DIR}
rm -rf ${LOCAL_JOB_DIR}/output/${seed}/ckpts/
mkdir -p ${LOCAL_JOB_DIR}/output/${seed}/ckpts/
mv ${LOCAL_JOB_DIR}/transfer-model-ep-31.pt ${LOCAL_JOB_DIR}/output/${seed}/ckpts/

cd ${LOCAL_JOB_DIR}
tar -czf ${fname}-intermediate.tgz output
cp -r ${fname}-intermediate.tgz ${RESULT_STORAGE_DIR}
echo "Results Copied"
echo "Done!"
