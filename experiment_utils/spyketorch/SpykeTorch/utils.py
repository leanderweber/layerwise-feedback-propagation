...
class LateralIntencityInhibition:
    ...
    def intensity_lateral_inhibition(self, intencities):
        ...
        patches = patches.reshape(
            patches.size(0), patches.size(1), patches.size(2), -1, patches.size(3) * patches.size(4)
        )
        patches.squeeze_(-2)
        # row-wise
...
class Intensity2Latency:
    ...
    def intensity_to_latency(self, intencities):
        ...
        # bin packing
        sorted_bins_value, sorted_bins_idx = (
            torch.split(intencities_flattened_sorted[0], bin_size),
            torch.split(intencities_flattened_sorted[1], bin_size),
        )
