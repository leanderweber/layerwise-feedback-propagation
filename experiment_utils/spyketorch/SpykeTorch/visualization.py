...
def get_deep_feature(pre_feature, feature_stride, window_size, stride, weights=None):
    ...
    if weights is None:  # place the feature in the middle of the field
        ...
        new_feature[
            :,
            start_point[0] : start_point[0] + pre_feature.size(-2),
            start_point[1] : start_point[1] + pre_feature.size(-1),
        ] = pre_feature
    else:
        for r in range(weights.size(-2)):  # rows
            for c in range(weights.size(-1)):  # cols
                ...
                new_feature[
                    :,
                    r * feature_stride[0] : r * feature_stride[0] + pre_feature.size(-2),
                    c * feature_stride[1] : c * feature_stride[1] + pre_feature.size(-1),
                ] += temp_features
