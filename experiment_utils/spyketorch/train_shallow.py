...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="ETH",
        help="Task to perform: Caltech, ETH, Norb",
        choices=["Caltech", "ETH", "Norb"],
    )
    ...
    if task == "Caltech":
        ...
        trainset = DataLoader(
            datafolder, batch_size=8 * 9 * 41, sampler=torch.utils.data.SubsetRandomSampler(train_indices)
        )
        testset = DataLoader(
            datafolder, batch_size=8 * 1 * 41, sampler=torch.utils.data.SubsetRandomSampler(test_indices)
        )
        max_epoch = 250
    elif task == "Norb":
    ...
    with open("results/category_perf_train.csv", "w") as f:
        f.write(
            "epoch,accuracy,loss,silence,correct_face,wrong_face,silence_face,correct_motor,wrong_motor,silence_motor\n"
        )
    ...
    for epoch in range(max_epoch):
        ...
        # S1 Features #
        if use_cuda:
            feature, cstride = vis.get_deep_feature(
                feature, cstride, (filter.max_window_size, filter.max_window_size), (1, 1), filter.kernels.cuda()
            )
            ...
            feature, cstride = vis.get_deep_feature(
                feature, cstride, (filter.max_window_size, filter.max_window_size), (1, 1), filter.kernels
            )
        # C1 Features #
        feature, cstride = vis.get_deep_feature(
            feature, cstride, (s1c1.pooling_size, s1c1.pooling_size), (s1c1.pooling_stride, s1c1.pooling_stride)
        )
        # save features to file
        for i in range(len(feature)):
