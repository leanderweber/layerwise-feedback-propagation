...
def eq1_a(a, W_next, b_next, z_next, u_next, rho):
    res = rho * torch.matmul(
        torch.transpose(W_next, 0, 1),
        torch.matmul(W_next, a) + b_next - z_next - u_next / rho,
    )
    return res
...
def eq1_b(a, W_next, b_next, z_next, u_next, rho):
    res = torch.reshape(
        torch.mean(
            rho * (torch.matmul(W_next, a) + b_next - z_next - u_next / rho), dim=1
        ),
        shape=(-1, 1),
    )
    return res
...
def P(W_new, theta, a_last, W, b, z, u, rho):
    temp = W_new - W
    res = (
        eq1(a_last, W, b, z, u, rho)
        + torch.sum(eq1_W(a_last, W, b, z, u, rho) * temp)
...
def Q(a_new, tau, a, W_next, b_next, z_next, u_next, v, z, rho):
    temp = a_new - a
    res = (
        a_obj(a, W_next, b_next, z_next, u_next, v, z, rho)
        + torch.sum(a_obj_gradient(a, W_next, b_next, z_next, u_next, v, z, rho) * temp)
        + torch.sum(tau * temp * temp) / 2
    )
    return res
...
def a_obj(a, W_next, b_next, z_next, u_next, v, z, rho):
    res = eq1(a, W_next, b_next, z_next, u_next, rho) + rho / 2 * torch.sum(
        (a - relu(z) + v) * (a - relu(z) + v)
    )
    return res
...
def update_W(a_last, b, z, W_old, u, rho, alpha=1):
    ...
    while eq1(a_last, zeta, b, z, u, rho) > P(zeta, alpha, a_last, W_old, b, z, u, rho):
        alpha = alpha * gamma
        zeta = (
            W_old - gradients / alpha
        )  # Learning rate decreases to 0, leading to infinity loop here.
    theta = alpha
    W = zeta
    return W
...
def z_obj(a_last, W, b, z, u, v, a, rho):
    f = (z - torch.matmul(W, a_last) - b + u / rho) * (
        z - torch.matmul(W, a_last) - b + u / rho
    ) + (a - relu(z) + v) * (a - relu(z) + v)
    return f
...
def update_zl(a_last, W, b, label, zl_old, u, rho):
    ...
    for i in range(MAX_ITER):
        ...
        fzl = cross_entropy_with_softmax(label, zl) + rho / 2 * torch.sum(
            (zl - torch.matmul(W, a_last) - b + u / rho)
            * (zl - torch.matmul(W, a_last) - b + u / rho)
        )
        ...
        zeta_old = zeta
        zeta = (
            rho * (torch.matmul(W, a_last) + b - u / rho)
            + (zl - eta * gradients2) / eta
        ) / (rho + 1 / eta)
...
def update_a(W_next, b_next, z_next, z, a_old, u_next, v, rho, t=1):
    ...
    while a_obj(beta, W_next, b_next, z_next, u_next, v, z, rho) > Q(
        beta, t, a_old, W_next, b_next, z_next, u_next, v, z, rho
    ):
        t = t * eta
        beta = a_old - gradient / t
    tau = t
