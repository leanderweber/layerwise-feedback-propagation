...
class mnist:
    ...
    def __init__(self, datapath):
        ...
        self.y_train = torch.eye(10, device=device)[self.y_train][
            :55000
        ]  # torch.tensor(to_categorical(self.y_train, num_classes=10).reshape(60000, 10)[:55000],device=device)

        self.x_test = self.x_test.reshape(10000, 28 * 28)
        self.y_test = torch.eye(10, device=device)[
            self.y_test
        ]  # torch.tensor(to_categorical(self.y_test, num_classes=10).reshape(10000, 10),device=device)
